$(document).ready(function() {

    var source = $("#beer-template").html();
    
    var template = Handlebars.compile(source);
    
	// This is just static sample data.  In a practical situation, this would be replaced with a database operation.
    var data = {beers: [
      {name: "Guinness", type: "Stout", website: "http://www.guinness.com", description: "Super delicious.  Better get a draught or it's not a Guinness."},
      {name: "Fat-Tire", type: "Amber Ale", website: "http://www.newbelgium.com", description: "My favorite Amber Ale.  Plus the picture is cool."},
      {name: "Sierra-Nevada", type: "Pale Ale", website: "http://www.sierranevada.com/beer", description: "The best Pale Ale.  Need I say more?"},
      {name: "Bud-Light", type: "Nearly Water", website: "http://www.budlight.com/", description: "This is almost water.  Stay hydrated!"}
   ]};
    
    var outMarkup = template(data);
    
    $("#beer-list").append(outMarkup);

    // defaults panel headers to an opacity of 50%
    $( ".panel-heading").css(
                            "opacity", "0.7"
    );
    $( ".panel-heading").on( "mouseleave", function(){
        $( ".panel-heading").fadeTo( 50, "0.7");
    });
    
    // Opacity set to 100% on mouseover
    $( ".panel-heading").on( "mouseover", function(){
        $( this).fadeTo( 120, "1");
    });
    // Expand beer details upon clicking on the beer name.
    $( ".panel-body").hide();
  
    $( ".panel-heading").on( "click", function() {
        var panelHeaderId = $(this).attr( "data-panelHeadingId");
        $( "#" + panelHeaderId).slideToggle(500);
    });
});